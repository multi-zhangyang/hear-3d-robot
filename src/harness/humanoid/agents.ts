...
function motionInstructions(): string {
  return [
    ...
    "termination 必须描述本轮可由当前传感状态验证的物理结果，例如根节点或身体 Link 到达位置、身体与可见物体接触、可见物体到达位置或区域。简单目标可使用全部谓词隐式 AND；复杂目标可用受限 all/any/not 条件树和 precondition/during/terminal 阶段。stable_steps 用于排除瞬时碰撞；不得引用当前不可见对象来宣称成功。",
    "每个 termination 谓词都填写完整字段集合：当前类型不使用的字段必须为 null。阶段条件用 predicate_indexes 表示正向谓词、not_predicate_indexes 表示否定谓词，再由 op=all 或 any 组合；不需要分阶段时 phases 填 null。",
