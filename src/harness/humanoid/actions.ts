...
export const HumanoidActionDescriptions: Record<HumanoidActionName, string> = {
  ...
  plan_whole_body_motion: "提交根运动、躯干朝向、双手腕与双脚踝末端连续位置组成的任务空间关键帧，以及可选的精确身体-物体接触约束。运动后端求解连续 G1 全身参考，运行时在当前状态上完整物理预演；末端不可达、碰错物体、失衡或跌倒会拒绝该计划。",
  plan_whole_body_motion_candidates: "提交受限 all/any/not 物理条件与可选 precondition/during/terminal 阶段，以及 2 至 3 个由模型按偏好排序的连续全身动作候选。时长只是最多 8 秒的执行上界；每个候选从同一 MuJoCo 状态完整预演，只有持续约束未被违反且终止条件稳定达成的候选才可能被选择。",
