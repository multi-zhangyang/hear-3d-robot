...
export function workerInstructions(spec: AgentSpec, runtime: HarnessRuntimeContext): string {
  ...
  return [
    ...
    "For an exploration cycle, call survey_terrain, compare its returned frontier entries, choose one exact choice_id, then call navigate_frontier with that survey transaction and choice. navigate_frontier validates and physically executes only that model-selected choice; it never chooses, substitutes, retries, or moves on its own. The next cycle surveys the changed world again.",
