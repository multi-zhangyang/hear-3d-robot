...
export function coordinatorInstructions(): string {
  return [
    ...
    "For exploration or roaming, create a genuine movement supervisor with enough authority to delegate repeated observe-plan-execute cycles. Give each executing leaf survey_terrain, plan_base_path, and execute_base_plan together so one model-run node owns one complete physical movement cycle; make the accepted execute_base_plan receipt the cycle's terminal evidence, while survey and plan remain its source trail. Retain at least one extra capability only at the supervisor so every child grant remains strictly narrower.",
...
export function workerInstructions(spec: AgentSpec, runtime: HarnessRuntimeContext): string {
  ...
  return [
    ...
    "For an exploration cycle, call survey_terrain, choose one of its returned frontier entries, call plan_base_path with that exact target and face_point, then execute the accepted plan. The next cycle surveys the changed world again. The harness supplies entropy and validates reachability but never chooses or performs the motion for you.",
    "A movement leaf should normally own survey_terrain, plan_base_path, and execute_base_plan in one grant, with one terminal execute_base_plan evidence criterion. Splitting those three across separate leaves adds handoffs without adding authority separation and commonly strands a valid plan.",
