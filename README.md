...
每个层级节点拥有独立的 Agents SDK Session。活动上下文接近配置阈值时，系统调用模型生成结构化压缩记录，并保留近期原始轮次。压缩结果必须引用真实动作回执；完整事件、模型生命周期、动作、具身经历、检查器和上下文记录继续保存在追加式日志中。每次模型请求只装载 Goal DAG 的当前工作集和近期 epoch，完整 DAG 仍保留在检查点；目标管理智能体可按状态、谓词、对象、方块、区域或候选标识分页召回更早的 Goal。模型生成的经历摘要不会进入权威状态块。协调与运动智能体可以按 `episode:N` 或 `action:<transactionId>` 精确召回成功、拒绝、漂移、约束违规和停滞记录；所有召回结果均标记为历史信息，不能代替当前传感。
...
`AI_REQUEST_TIMEOUT_MS` 默认是 `300000`，用于容纳长思考与复杂工具规划；可按端点能力在 5 秒至 10 分钟之间调整。任务总时限、人工停止和进程恢复仍独立生效，不会因单次请求时限而失去可中断性。
