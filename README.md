...
标准运动链只接收语义 Skill。模型决定探索 frontier、操作对象、交互点、左右手和策略，求解层再从当前腕部碰撞面、对象尺寸、关节轴、铰链锚点、持物绑定和根姿态计算站位与末端目标。多末端 SE(3) 阻尼最小二乘逆运动学生成连续腿臂参考，YAHMP 神经残差与任务链阻抗组成的混合控制器负责跟踪。接近、伸手、抓取、抬升、搬运、放置、推拉、按压、开合、换手、双手支撑、稳定、撤退和方块交互使用同一套 Skill 约束，不依赖场景专用动作表。
