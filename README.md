...
自主探索采用“勘察—模型选择—原子导航”协议。`survey_terrain` 返回当前世界版本下由 Recast 验证可达的边界候选，模型明确选择其中一个 `choice_id` 后调用 `navigate_frontier`。Harness 只执行该候选，并验证勘察回执归属、候选身份和世界版本；它不会替模型选择方向、换用其他候选或自动重试。一般目标导航仍使用独立的路线规划与执行回执。
