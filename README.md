...
3. 任务空间目标能够生成合法的连续全身参考。
4. 完整 MuJoCo 预演没有跌倒、非法接触或缺失的必需接触。
5. 执行结束后的权威状态通过回执和任务谓词检查。

自主差异来自模型对实时观察、目标和历史回执的采样决策，以及每次任务独立生成的世界。程序不会从预设动作表挑选行为，也不会用随机电机噪声代替自主决策。

## 机器人与世界

默认机器人使用 Unitree G1 模型，身体控制按四个通道组织：

| 通道 | 范围 |
|---|---|
| `locomotion` | 根速度、朝向、双腿与足部支撑 |
| `torso` | 腰部与躯干姿态 |
| `left_arm` | 左臂和左手末端 |
| `right_arm` | 右臂和右手末端 |

运动工具接收任务空间关键帧。根运动使用身体局部速度，双手目标可使用世界坐标或骨盆相对坐标；关节参考由运动生成器求解并交给 YAHMP 跟踪。模型可以组合行走、转身、躯干调整和双臂动作，而不需要依赖固定动作名称。
...
每个层级节点拥有独立的 Agents SDK Session。活动上下文接近配置阈值时，系统调用模型生成结构化压缩记录，并保留近期原始轮次。压缩结果必须引用真实动作回执；完整事件、模型生命周期、动作、检查器和上下文记录继续保存在追加式日志中。
