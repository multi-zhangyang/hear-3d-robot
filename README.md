...
外接学习策略未声明 `joint_reference_tracking` 时，模拟器会把它作为主控制器，并创建独立 YAHMP 参考控制器组成能力路由。无关节参考的平衡和移动继续由主策略推理；任务需要主策略未声明的能力或出现关节参考时，路由从当前 MuJoCo 状态重置并调用参考控制器。两个分支使用相同控制周期，共同保存到物理检查点；主策略的 `learnedPolicy.capabilities` 保持原值，参考控制不会被合并或冒充为训练能力。已经完整实现能力路由的外接控制器可以通过 `capabilityRouting` 描述自身边界，运行时不会再次包装。
