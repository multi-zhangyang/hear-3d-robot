...
import { loadEnvironment, loadProviderConfig, loadRuntimeCatalog } from
  "../dist/config/load.js";
...
import { inspectLiveManipulationEvidence } from "./live-manipulation-evidence.mjs";

loadEnvironment();

const scenarioId = optionalText("HEAR_LIVE_SCENARIO") ?? "humanoid_workyard";
...
const result = await startHumanoidMission({
  runsDir,
  mission: "在当前世界中自主完成场景定义的物体搬运与稳放目标。",
  scenarioId,
...
assert.equal(
  ...
  "Manipulation run did not retain the configured humanoid controller source"
);
const evidence = await inspectLiveManipulationEvidence({ store, scenario });
